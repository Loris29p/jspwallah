...
function generateTeams(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const teamTypes = ['solo', 'duo', 'trio', 'league'];
        const randomType = teamTypes[Math.floor(Math.random() * teamTypes.length)];
        
        // Get max players based on team type
        const maxPlayers = randomType === 'solo' ? 1 : (randomType === 'duo' ? 2 : (randomType === 'trio' ? 3 : 20));
...
function joinTeam(teamId) {
    ...
    if (Type === "league") {
        ...
        $.post("https://gamemode/host:joinTeam", JSON.stringify({
            teamId: team.serverId,
        }));
    }
...
function leaveTeam(teamId, shouldNotifyServer = true) {
    ...
    if (shouldNotifyServer) {
        if (Type === "league") {
            ...
            $.post("https://gamemode/host:leaveTeam", JSON.stringify({
                teamId: team.serverId,
            }));
        }
    }
...
$(document).on("keyup", function (e) {
    if (e.key === "Escape") {
        if (Type === "league") {
            ...
            $.post("https://gamemode/closePanelHost", JSON.stringify({}));
        }
    }
});
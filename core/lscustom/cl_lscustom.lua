...
IsInCustomVehicle = false

local garages = {
	[1] = vector4(1384.452, -577.3889, 74.3382, 291.0076), -- Mirror Park
    [2] = vector4(242.3443, -1411.035, 30.58755, 142.3823), -- Hospital
    [3] = vector4(1158.289, -1494.209, 34.69263, 91.01795), -- Marabunta
    [4] = vector4(-539.3762, -245.4214, 36.15752, 171.8123), -- Main
    [5] = vector4(-433.4088, 1142.618, 325.9036, 345.9933), -- Mountain
    [6] = vector4(-939.6505, 6203.171, 3.307337, 123.952), -- Paleto
    [7] = vector4(1482.564, 6366.088, 23.70888, 117.7659), -- Hideout
    [8] = vector4(2775.573, 3459.97, 55.54049, 228.9646), -- Sandy Shores
    [9] = vector4(-1089.815918, -1247.935791, 5.194950, 111.986732), -- Beach
	[9] = vector4(773.00, -1408.67, 26.65, 266.04), -- Depot

	-- 	[1] = { locked = false, camera = {x = -418.6, y = 1206.78, z = 325.37, heading = 229.34},
	-- 	driveout = {x = -418.6, y = 1206.78, z = 325.37, heading = 229.34},
	-- 	drivein = {x = -418.6, y = 1206.78, z = 325.37, heading = 229.34},
	-- 	outside = {x = -418.6, y = 1206.78, z = 325.37, heading = 229.34},
	-- 	inside = {x = -418.6, y = 1206.78, z = 325.37, heading = 229.34}},
...
_RegisterNetEvent('LSC:installMod')
_AddEventHandler('LSC:installMod', function()
	local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
	myCar = GetVehicleData(vehicle) --ESX.Game.GetVehicleProperties(vehicle)
	SetVehicleData(vehicle, myCar)
	Tse('LSC:refreshOwnedVehicle', myCar)
end)
...
_RegisterNetEvent('LSC:cancelInstallMod')
_AddEventHandler('LSC:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)
	--ESX.Game.SetVehicleProperties(vehicle, myCar)
	Core.Game.SetVehicleData(vehicle, myCar)
end)
...
local function DriveInGarage()
	--myveh.repair()
	--Lock the garage
	--Tse('lockGarage',true,currentgarage)
    IsInCustomVehicle = true
	...
	if DoesEntityExist(veh) then
		LocalPlayer.state:set("invisibleAll", true, false)
		...
		elseif type(LSC_Config.menu.theme) == "string" then
			if LSC_Config.menu.theme == "light" then
				...
				LSCMenu:setColors({ r = 255, g = 255, b = 255, a = 255 }, { r = 0, g = 0, b = 0, a = 255 },
					{ r = 0, g = 0, b = 0, a = 155 }, { r = 255, g = 255, b = 255, a = 255 })
			elseif LSC_Config.menu.theme == "darkred" then
				LSCMenu:setColors({ r = 255, g = 255, b = 255, a = 255 }, { r = 0, g = 0, b = 0, a = 255 },
					{ r = 0, g = 0, b = 0, a = 155 }, { r = 200, g = 15, b = 15, a = 200 })
			elseif LSC_Config.menu.theme == "bluish" then
				LSCMenu:setColors({ r = 255, g = 255, b = 255, a = 255 }, { r = 255, g = 255, b = 255, a = 255 },
					{ r = 0, g = 0, b = 0, a = 100 }, { r = 0, g = 100, b = 255, a = 200 })
			elseif LSC_Config.menu.theme == "greenish" then
				LSCMenu:setColors({ r = 255, g = 255, b = 255, a = 255 }, { r = 0, g = 0, b = 0, a = 255 },
					{ r = 0, g = 0, b = 0, a = 100 }, { r = 0, g = 200, b = 0, a = 200 })
			end
		end
		...
		-- AddMod(16, LSCMenu.categories, "ARMOR", "Armor",
		-- 	"Protect your car's occupants with military spec composite body panels.", true)
		...
		if interior then
			...
			AddMod(33, LSCMenu.categories.Interior, "STEERING WHEELS", "Steering Wheels",
				"Customize the link between you and your vehicle.", true)
		...
		for n, c in pairs(LSC_Config.prices.utility.colors) do
			local btn = pcol.Utility:addPurchase(c.name, LSC_Config.prices.utility.price)
			btn.colorindex = c.colorindex
			if btn.colorindex == myveh.color[1] then
				btn.purchased = true
			end
		...
		for n, c in pairs(LSC_Config.prices.worn.colors) do
			local btn = pcol.Worn:addPurchase(c.name, LSC_Config.prices.worn.price)
			btn.colorindex = c.colorindex
			if btn.colorindex == myveh.color[1] then
				btn.purchased = true
			end
		...
		for n, c in pairs(LSC_Config.prices.pearlescent.colors) do
			local btn = pcol.Pearlescent:addPurchase(c.name, LSC_Config.prices.pearlescent.price)
			btn.colorindex = c.colorindex
			if btn.colorindex == myveh.color[1] and myveh.extracolor[1] == myveh.color[2] then
				btn.purchased = true
			end
		...
		for n, c in pairs(LSC_Config.prices.chameleon.colors) do
			local btn = pcol.Chameleon:addPurchase(c.name, LSC_Config.prices.chameleon.price)
			btn.colorindex = c.colorindex
			if btn.colorindex == myveh.color[1] then
				btn.purchased = true
			end
		...
		for n, c in pairs(LSC_Config.prices.utility2.colors) do
			local btn = scol.Utility:addPurchase(c.name, LSC_Config.prices.utility2.price)
			btn.colorindex = c.colorindex
			if btn.colorindex == myveh.color[2] then
				btn.purchased = true
			end
		...
		for n, c in pairs(LSC_Config.prices.worn2.colors) do
			local btn = scol.Worn:addPurchase(c.name, LSC_Config.prices.worn2.price)
			btn.colorindex = c.colorindex
			if btn.colorindex == myveh.color[2] then
				btn.purchased = true
			end
		...
		for n, c in pairs(LSC_Config.prices.pearlescent2.colors) do
			local btn = scol.Pearlescent:addPurchase(c.name, LSC_Config.prices.pearlescent2.price)
			btn.colorindex = c.colorindex
			if btn.colorindex == myveh.color[2] and myveh.extracolor[1] == btn.colorindex then
				btn.purchased = true
			end
		...
		for n, c in pairs(LSC_Config.prices.chameleon2.colors) do
			local btn = scol.Chameleon:addPurchase(c.name, LSC_Config.prices.chameleon2.price)
			btn.colorindex = c.colorindex
			if btn.colorindex == myveh.color[2] then
				btn.purchased = true
			end
		...
		AddMod(13, LSCMenu.categories, "TRANSMISSION", "Transmission",
			"Improved acceleration with close ratio transmission.", true)
		...
		wtype = LSCMenu.categories.Wheels:addSubMenu("WHEEL TYPE", "Wheel Type", "Custom rims in all styles and sizes.",
			true)
		if IsThisModelABike(GetEntityModel(veh)) then
			...
			-- 	local btn = customw:addPurchase(w.name, w.price)
			-- 	btn.wtype = w.wtype
			-- 	btn.modtype = 23
			-- 	btn.mod = w.mod
			-- end
		...
		-- m = LSCMenu.categories.Wheels:addSubMenu("WHEEL ACCESSORIES", "Tires",
		-- 	"Custom burnout smoke.", true)
...
local function DriveOutOfGarage(pos)
	Citizen.CreateThread(function()
        ...
        IsInCustomVehicle = false
		pos = currentpos
		TaskVehicleDriveToCoord(ped, veh, pos.outside.x, pos.outside.y, pos.outside.z, f(5), f(0.1), GetEntityModel(veh),
			16777216, f(0.1), true)

		pos = currentpos.driveout

		--The vehicle customization is finished, so we send to server our vehicle data
		Tse("LSC:finished", myveh)
		...
		--Unlock the garage
		--Tse('lockGarage',false,currentgarage)
...
Citizen.CreateThread(function()
	while true do
		local sleep = 1000
		--local ped = LocalPed()
		--local pCoords = LocalPlayer.state.coords
		local ped = PlayerPedId()
		local pCoords = GetEntityCoords(ped)
		if GM.Player.InSafeZone then
			
			for i, pos in pairs(garages) do
				distance = #(vector3(pos.x, pos.y, pos.z) - vector3(pCoords.x, pCoords.y, pCoords.z))
				if distance < 50 and distance > 3 then
					sleep = 0
					DrawMarker(36, pos.x, pos.y, pos.z, 0, 0, 0, 0, 0, 0, 2.5, 2.5, 2.5, 0, 255, true, 200, 0, 0, 0, true)
				end
				if IsPedSittingInAnyVehicle(ped)  then
					local veh = GetVehiclePedIsUsing(ped)

					if distance < 3 and DoesEntityExist(veh) and GetPedInVehicleSeat(veh, -1) == ped then
						sleep = 0
						if IsControlPressed(0, 74) and not tableContains(LSC_Config.ModelBlacklist, GetDisplayNameFromVehicleModel(GetEntityModel(veh)):lower()) then
							-- Tse("lscustom:server:enterchangeBucket", veh)
							Tse("bucketlscustom", "join", NetworkGetNetworkIdFromEntity(veh))
                            oldPos = GetEntityCoords(PlayerPedId())
                            DoScreenFadeOut(500)
							Citizen.Wait(1000)
                            SetEntityCoords(veh, -597.5236, -717.0681, 131.0401)
                            SetEntityCoords(PlayerPedId(), -597.5236, -717.0681, 131.0401)
                            Citizen.Wait(200)
							...
							SetVehicleFixed(veh)
							DriveInGarage()
							-- Tse("bucketlscustom", "join", veh)
						--else
							--drawTxt("Press [~b~X~s~] to enter the ~b~Vehicle Workshop~s~.", 4, 1, 0.5, 0.8, 1.0, 255, 255,255, 255)
						end
                        DrawTopNotification("Presss ~INPUT_VEH_HEADLIGHT~ to enter the ~r~Vehicle Custom~s~.", true)
						
					end
				elseif distance < 3 then
					sleep = 0
                    DrawTopNotification("~r~You must in a vehicle!", true)
				end
			end
		end
		Citizen.Wait(sleep)
...
function LSCMenu:OnMenuClose(m)
	local plyVeh = LocalPlayer.state.inVehicle
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	Tse('LCS_SaveVehicle', GetVehicleData(vehicle))
    DoScreenFadeOut(500)
    Citizen.Wait(500)
    SetEntityCoords(PlayerPedId(), oldPos)
    SetEntityCoords(vehicle, oldPos)
    Citizen.Wait(500)
    SetPedIntoVehicle(PlayerPedId(), vehicle, -1)
    Citizen.Wait(3000)
    DoScreenFadeIn(500)
	FreezeEntityPosition(vehicle, false)
	LocalPlayer.state:set("invisibleAll", false, false)
	Tse("bucketlscustom", "leave", NetworkGetNetworkIdFromEntity(vehicle))
	
	--print(Core.Shared.Vehicles[GetEntityModel(plyVeh)], plyVeh , Core.Shared.Vehicles[plyVeh])


	-- Tse('LCS_SaveVehicle', GetVehicleData(plyVeh))
	-- DriveOutOfGarage(currentpos.outside)


	-- Tse("lscustom:server:exitchangeBucket", plyVeh)
end
...
function LSCMenu:onButtonSelected(name, button)
	...
	--Tse("LSC:buttonSelected", name, button)
	ButtonSelected(name, button, true)
end

--So we get the button back from server +  bool that determines if we can prchase specific item or not

function ButtonSelected(name, button, canpurchase)
	-- 	if button.args ~= nil and next(button.args) ~= nil then

	-- 	SeVehicleData(vehicle, button.args)
	-- end
	-- DecorSetInt(vehicle, "id", 31)
	...
	elseif mname == "sport" or mname == "muscle" or mname == "lowrider" or mname == "back wheel" or mname == "front wheel" or mname == "highend" or mname == "suv" or mname == "offroad" or mname == "tuner" or mname == "street" or mname == "track" or mname == "bb" or mname == "bo" or mname == "race" then
		if button.purchased or CanPurchase(price, canpurchase) then
...
function LSCMenu:OnMenuChange(last, current)
	...
	elseif c == "front wheel" or c == "wheel accessories" or c == "wheel color" or c == "sport" or c == "muscle" or c == "lowrider" or c == "highend" or c == "suv" or c == "offroad" or c == "tuner" or c == "street" or c == "track" or c == "bb" or c == "bo" or c == "race" then
...
function CheckPurchases(m)
	...
	elseif name == "sport" or name == "muscle" or name == "lowrider" or name == "back wheel" or name == "front wheel" or name == "highend" or name == "suv" or name == "offroad" or name == "tuner" or name == "street" or name == "track" or name == "bb" or name == "bo" or name == "race" then
		for i, b in pairs(m.buttons) do
...
-- 		local blip = AddBlipForCoord(pos.x,pos.y,pos.z)--(pos.inside.x,pos.inside.y,pos.inside.z)
...
-- local firstspawn = 0
-- _AddEventHandler('playerSpawned', function(spawn)
...
-- _RegisterNetEvent('lockGarage')
-- _AddEventHandler('lockGarage', function(tbl)

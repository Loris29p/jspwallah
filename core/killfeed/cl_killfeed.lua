weaponsHashs = {
    ...
    [tonumber(GetHashKey('weapon_precisionrifle'))] = 'weapon_precisionrifle',
}

function hashToWeapon(hash)
    print(hash, GetHashKey('weapon_precisionrifle'))
	if weaponsHashs[hash] ~= nil then
		return weaponsHashs[hash]
	else
...
_RegisterNetEvent("gamemode:killfeed:sendEvent")
_AddEventHandler("gamemode:killfeed:sendEvent", function(killer, victim, weapon, isRedzone, prestige, killerPrestige, isFFA)
    SendNUIMessage({
        type = "killfeed",
        killer = killer,
        victim = victim,
        weapon = weapon,
		isRedzone = isRedzone and true or false,
		prestige = (prestige) or 0,
		killerPrestige = (killerPrestige) or 0,
        isFFA = (isFFA and true or false)
    })
end)
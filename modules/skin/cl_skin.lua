...
function LoadDefaultModel(loadMale, cb)
	...
	Citizen.CreateThread(function()
		...
		if IsModelInCdimage(characterModel) and IsModelValid(characterModel) then
			...
			-- Ensure max health and armor are set correctly for all models
		...
		_TriggerEvent('skinchanger:modelLoaded')
...
_AddEventHandler('skinchanger:loadDefaultModel', function(loadMale, cb)
	LoadDefaultModel(loadMale, cb)
end)

_AddEventHandler('skinchanger:getData', function(cb)
...
_AddEventHandler('skinchanger:change', function(key, val)

	Character[key] = val
	print("VAL", val)

	if key == 'sex' then
		_TriggerEvent('skinchanger:loadSkin', Character)
	else
		ApplySkin(Character)
	end
...
_AddEventHandler('skinchanger:getSkin', function(cb)
	cb(Character)
end)

_AddEventHandler('skinchanger:modelLoaded', function()
...
_RegisterNetEvent('skinchanger:loadSkin')
_AddEventHandler('skinchanger:loadSkin', function(skin, cb)
	-- local playerPed = GetPlayerPed(-1)
	-- local characterModel
	...
	-- 	-- print(skin.sex)
	-- 	if skin['sex'] == 0 then
	-- 		_TriggerEvent('skinchanger:loadDefaultModel', true, cb)
	-- 	elseif skin['sex'] > 1 then
	-- 		characterModel = pedList[skin.sex - 1]
	-- 	else
	-- 		_TriggerEvent('skinchanger:loadDefaultModel', false, cb)
	-- 	end
	-- 	RequestModel(characterModel)
	-- else
	-- 	ApplySkin(skin)
...
_RegisterNetEvent('skinchanger:loadClothes')
_AddEventHandler('skinchanger:loadClothes', function(playerSkin, clothesSkin)
	local playerPed = GetPlayerPed(-1)
	local characterModel
	if playerSkin['sex'] ~= LastSex then
		...
		if playerSkin['sex'] == 0 then
			_TriggerEvent('skinchanger:loadDefaultModel', true)
		elseif playerSkin['sex'] > 1 then
			characterModel = pedList[playerSkin.sex - 1]
		else
			_TriggerEvent('skinchanger:loadDefaultModel', false)
		end
		RequestModel(characterModel)
	else
...
--         _TriggerEvent('skinchanger:getSkin', function(skin)
--             Tse("kframework:SaveSkinPlayer", skin)
--         end)
